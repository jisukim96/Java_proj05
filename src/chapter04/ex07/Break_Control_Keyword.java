...
public class Break_Control_Keyword {
	...
	public static void main(String[] args) {
		...
		for (int i = 0 ; i < 10 ; i ++) {
			System.out.println(i);
			if(i ==5) {					// if 조건을 사용해서 조건이 만족되었을 때 탈출
				break;
			}
		}			// 0, 1, 2, 3, 4, 5 
		...
		//	case 2 : 점프해서 한번에 여러 바깥쪽에 for문을 탈출하는 경우 (LABLE) 
		...
		//	case 2 : 점프해서 한번에 여러 바깥쪽에 for문을 탈출하는 경우 (변수 값을 조정해서 탈출)
			
			for( int i = 0; i < 5; i++) {
				for (int j = 0 ; j < 5; j++) {
					if (j==2) {
						i = 100;		// 바깥쪽 for문의 변수의 값을 다른 값을 넣어 조건을 false로 만듦.
						break;
					}
					System.out.println(i + " , "+j);
				}
