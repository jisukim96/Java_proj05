...
class C{
	int a = 10;				// 인스턴스 필드 : 객체화해야 사용 가능, 다른 객체와는 공유되지않는 값.
								// 메모리의 Heap 공간에 변수와 값을 저장
	static int b = 20;		// 정적 필드 : 객체화하지않고 사용 가능.
									// 모든 객체에서 공유되는 변수 
									// 메모리의 클래스 영역에 변수의 값을 저장, 
									// Heap 영역에는 클래스 영역의 저장된 주소 값만 가진다.
...
public class Instance_VS_Static02 {
	...
	public static void main(String[] args) {
		...
		C c1 = new C();
		// 객체 필드의 값 수정
		c1.a = 100;				// 인스턴스 필드 : 자신의 객체에만 적용됨. 다른 객체와 공유되지않음. 
		c1.b = 777;				// 정적 필드 : 다른 객체와 공유되는 변수
		...
		c3.a = 777;
		c1.b = 111;
		C.b = 333;		//클래스 이름으로 바로 수정/ 모든 객체에서 공유되는 블락
				
		System.out.println("=============================");
		System.out.println(c1.a);	// 인스턴스 필드 : 다른 객체와 분리된 고유공간
		System.out.println(c2.a);	// 인스턴스 필드 :  
		System.out.println(c3.a);
		System.out.println(c1.b);	// 정적 필드 : 모든 인스턴스(객체)에서 공유되는 공간 (static)
