...
abstract class Dog extends Animal { // 추상 클래스의 추상메소드를 일부만 구현한 경우 : 추상 클래스
	@Override
	void cry() {
...
class DogChild extends Dog { // 콘크리트 클래스 , Animal class의 완전 메소드 구현 , 객체화하여 Animal 메소드 구현가능
	@Override
	void run() {
...
//		Animal a1 = new Animal();	오류 발생 : 추상클래스는 객체화하지 못한다.

//		Cat을 객체화해서 Animal 타입으로 정의 ,
//		Animal의 메소드 호출 시 오버라이딩된 Cat의 메소드 호출
		Animal a2 = new Cat();
		a2.eat();
		a2.run();
		a2.cry();
...
//		DogChild를 객체화해서 Animal 타입으로 지정
...
//		DogChild를 객체화 해서 Dog 타입으로 지정
		...
		System.out.println("==================");
//		DogChild를 객체화해서 DogChild 타입으로 지정
