...
class Human {	// 부모클래스 (Super class) : 자식 클래스의 공통된 특징을 저장 (필드 , 메소드)
	
	// 필드 ( 속성 )
	String name;		// 사람 이름
	int age;			// 사람 나이
	
	// 메소드 : 프로그래밍의 기능을 구현해놓은 것
	void eat() {
...
//자식 클래스 : 부모 클래스의 모든 필드와 메소드를 물려받게됨 
class Student extends Human{
	// 부모 클래스 : Human
	// 자식 클래스 : Student	<== 부모클래스의 필드와 메소드를 물려받는다.2
	
	// 1. 부모클래스의 모든 필드와 메소드를 그대로 자식 클래스 내부에 들어와있다.
...
public class Inheritance01 {
	...
	public static void main(String[] args) {
		//  상속 (Inheritance) : 부모 클래스의 필드(속성)와 메소드(기능)를 자식 클래스에서 물려받아 사용함
			// 객체 지향 언어의 꽃 (제일 중요함), 
			// 중복된 코드를 줄여줄 수 있다.
