...
public class LogicalOperator {
	...
	public static void main(String[] args) {
		...
		// 2. 비트 연산자로 논리 연산이 가능하다.	<=== << 쇼트 서킷을 발생하지 않도록 처리 >>
		// 		AND ( & ) , OR ( | ) , XOR ( ^ ) , NOT ( ~ )
		System.out.println( true & true );  // true
		System.out.println( true | false ); // true
		...
		// 3. 쇼트 서킷 : 논리 연산자에서 앞에 내용으로 뒤의 값의 유무없이 식별 가능하면 뒤의 값은 처리하지 않는다.
		
		System.out.println( true && true );    // 쇼트 서킷 발생되지 않는다.앞에 true , 뒤의 값 반드시 확인
		System.out.println( false && true );   // 쇼트 서킷 발생
			// AND 연산자에서는 처음 값이 false 가 오게되면 뒤에 어떤 값이 오더라도 결과는 false.
		
		System.out.println( false|| true); 	  // 쇼트 서킷 발생 안함
		...
		int value2 = 3;
		System.out.println( false & ++value2 > 6);  // 쇼트 서킷 발생 안됨  ( 비트연산자 : & )
		...
		int value4 = 3;
		System.out.println( true | ++value4 > 6);  // 쇼트 서킷 발생 안됨  ( 비트연산자 : | )
 		System.out.println( value4 );	 // 4
		
		// XOR ( ^ ) : 논리연산 , 비트연산 <== 뒤의 값을 반드시 확인해야 결과를 알 수 있으므로 쇼트 서킷 발생 X.
 			// 앞 뒤 값이 같으면 false , 값이 다르면 true
